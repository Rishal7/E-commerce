<x-layout>
    <x-setting heading="Manage Scheduled Exports">

        @if ($exports->where('executed', false)->isEmpty())
            <p class="text-gray-600 text-sm">No exports scheduled.</p>
        @else
                        ...
                        @foreach ($exports as $export)
                            @if (!$export->executed)
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <form method="POST" action="/admin/schedule-export/{{ $export->id }}">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <div class="bg-white rounded-lg shadow relative dark:bg-gray-700">
                                            ...
                                            <div class="px-6 py-6 lg:px-8">
                                                ...
                                                <form class="space-y-6" method="POST"
                                                    action="/admin/schedule-export/{{ $export->id }}"
                                                    enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PATCH')
                                                    <x-form.input name="date" type="datetime-local"
                                                        :value="old('date', $export->date)" />
